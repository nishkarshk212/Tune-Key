# Tune-Key (YTKey.io)

**YTKey.io / TuneKey API** is a full-stack SaaS platform and high-speed API gateway designed specifically for **Telegram Music Bots** (*YukkiMusic*, *AnonXMusic*, *PyTgCalls*, *Victoria*, etc.) to purchase, manage, monitor, and generate dedicated, high-quota YouTube API keys with direct Opus audio streaming relays and anti-429 ban protection.

---

## 🌟 Key Features

- **Dynamic Landing Page**: Modern dark obsidian aesthetics, purple gradient glows, 3D holographic hero pedestal, 5-card feature grid, and transparent 4-tier pricing matrix.
- **Interactive Global Sign-In Panel**: 1-click Google sign-in, demo credentials, and direct dashboard transitions.
- **User Dashboard**:
  - Live API usage line chart (Chart.js) and 24-hour circular usage progress ring.
  - 4 telemetry cards: Total API Keys, Requests Today, Plan (Pro Plan), and Credit Balance.
  - Complete API key management: generate, copy, reveal, activate/deactivate, and whitelisting.
- **Plans & Order Summary**: Multi-method checkout with instant automated key provisioning.
- **Bot Config Wizard**: Ready-to-deploy `.env` and `config.py` export for Yukki, AnonX, and PyTgCalls.
- **Admin Control Portal**: Platform metrics, user account controls, global key inspector, and real-time traffic audit logs.

---

## 🚀 Quickstart

### 1. Install Dependencies
```bash
npm install
```

### 2. Run the Development Server
```bash
npm run dev
```

### 3. Production Build & Server
```bash
npm run build
node server/index.js
```
Open **`http://localhost:5000`** in your browser.

---

## 🔑 Demo Accounts

| Role | Email | Password |
|---|---|---|
| **Demo User** | `demo@ytkey.io` | `Demo@1234` |
| **Super Admin** | `admin@ytkey.io` | `Admin@1234` |
