...
export default function Overview() {
  ...
  const handleAutoGenerateFreeKey = async () => {
    try {
      ...
      showNotification(res.data?.message || 'Free API Key generated successfully!', 'success');
      fetchStats();
    } catch (err) {
      showNotification(err.response?.data?.error || 'Failed to create free key', 'error');
    } finally {
  ...
  return (
    <div className="space-y-8 max-w-7xl mx-auto">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
        ...
        <Link
          to="/dashboard/keys"
          className="p-5 rounded-2xl bg-white dark:bg-[#11131B] border border-slate-200 dark:border-white/[0.07] hover:border-purple-500/50 flex items-center justify-between shadow-sm transition-all group cursor-pointer"
        >
          <div>
            <p className="text-xs font-semibold text-slate-500 group-hover:text-purple-400 transition-colors">Total API Keys</p>
            <p className="text-2xl font-bold text-slate-900 dark:text-white mt-1">
              {activeKeysCount}
            </p>
            <p className="text-[11px] text-purple-500 font-bold mt-0.5 flex items-center space-x-1">
              <span>Manage Keys</span>
              <span>→</span>
            </p>
          </div>
          <div className="w-10 h-10 rounded-xl bg-blue-500/10 group-hover:bg-purple-500/20 flex items-center justify-center text-blue-400 group-hover:text-purple-400 group-hover:scale-110 transition-all">
      ...
      <div className="p-6 rounded-2xl bg-white dark:bg-[#11131B] border border-slate-200 dark:border-white/[0.07] space-y-4">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-3 border-b border-slate-100 dark:border-white/[0.06]">
          ...
          <div className="flex items-center space-x-2">
            <Link
              to="/dashboard/keys"
              className="px-3.5 py-2 rounded-xl bg-slate-100 dark:bg-white/[0.06] hover:bg-slate-200 dark:hover:bg-white/[0.1] text-slate-700 dark:text-slate-200 text-xs font-bold flex items-center space-x-1.5 transition-all"
            >
              <Key className="w-3.5 h-3.5 text-purple-400" />
              <span>Manage All Keys</span>
            </Link>
            <Link
              to="/dashboard/plans"
